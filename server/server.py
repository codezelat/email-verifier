...
# Default to HTTPS. Free Mailboxlayer tier requires HTTP; paid tiers support HTTPS.
# Set MAILBOXLAYER_BASE_URL=http://apilayer.net/api/check for free tier.
MAILBOXLAYER_BASE_URL = os.getenv(
    "MAILBOXLAYER_BASE_URL", "https://apilayer.net/api/check"
)
REQUEST_TIMEOUT = int(os.getenv("REQUEST_TIMEOUT", "30"))
MAX_BULK_SIZE = 50
BULK_RATE_LIMIT_DELAY = float(os.getenv("BULK_RATE_LIMIT_DELAY", "0.2"))
...
class EmailVerificationService:
    ...
    def __init__(self, api_key: str, base_url: str = MAILBOXLAYER_BASE_URL):
...
verification_service = EmailVerificationService(
    MAILBOXLAYER_API_KEY, MAILBOXLAYER_BASE_URL
)
...
def verify_email():
    ...
    logger.info(
        f"Verification complete for {email}: {status} ({processing_time:.2f}s)"
    )
...
def bulk_verify_emails():
    ...
    if len(emails) > MAX_BULK_SIZE:
        return jsonify({
            "error": f"Bulk limit exceeded",
            "details": f"Received {len(emails)} emails. Maximum allowed is {MAX_BULK_SIZE} per request.",
            "suggestion": f"Split your list into chunks of {MAX_BULK_SIZE} and send multiple requests.",
        }), 413
    ...
    for email in emails:
        ...
        # Rate limiting between requests to avoid hitting API limits
        time.sleep(BULK_RATE_LIMIT_DELAY)

    processing_time = time.time() - start_time
    logger.info(
        f"Bulk verification complete: {len(results)} emails in {processing_time:.2f}s"
    )
...
if __name__ == "__main__":
    ...
    logger.info(f"Mailboxlayer URL: {MAILBOXLAYER_BASE_URL}")
